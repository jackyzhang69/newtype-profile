...
describe("empty-task-response-detector", () => {
  ...
  describe("tool.execute.after", () => {
    ...
    // #given a chief_task tool with empty response
    // #when the hook processes the output
    // #then it should inject the warning message
    it("should detect empty response from chief_task tool", async () => {
      const input = { tool: "chief_task", sessionID: "ses_123", callID: "call_2" }
      const output = { title: "chief_task", output: "", metadata: {} }
    ...
    // #given a chief_task tool with only whitespace response
    // #when the hook processes the output
    // #then it should inject the warning message
    it("should detect whitespace-only response as empty", async () => {
      const input = { tool: "chief_task", sessionID: "ses_123", callID: "call_3" }
      const output = { title: "chief_task", output: "   \n\t  ", metadata: {} }
    ...
    // #given a chief_task tool with valid response
    // #when the hook processes the output
    // #then it should NOT modify the output
    it("should not modify valid response", async () => {
      const input = { tool: "chief_task", sessionID: "ses_123", callID: "call_4" }
      const originalOutput = "Task completed successfully with results..."
      const output = { title: "chief_task", output: originalOutput, metadata: {} }
    ...
    it("should handle tool name case-insensitively", async () => {
      const testCases = ["TASK", "Task", "task", "CHIEF_TASK", "Chief_Task"]
